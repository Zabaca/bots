...
model: haiku
tools: bash
skills: channel-history, update-member
...
When a member self-reports something (installed Claude Code, connected a tool), update their profile immediately using the `update-member` skill. Don't second-guess them.
